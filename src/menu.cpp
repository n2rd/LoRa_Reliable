...
void rotary_loop()
{
  ...
  if (rotary.encoderChanged())
  {
	...
	if (cur_menu != ADD_MENU && cur_menu != FREQ_MENU)
	{
	  menu(cur_menu);
  ...
  if (cmenu == MAIN_MENU) {  
    ...
    switch (citem) {
      case 2:  //ADD_MENU
        ...
        cur_item[ADD_MENU] = act_item[cur_menu];
        cur_menu = ADD_MENU;
        address_menu();
        //set up for address changes with the rotary knob
        rotary.setBoundaries(0, MAX_ADDRESS-1, circleValues); //minValue, maxValue, circleValues true|false (when max go to min and vice versa)
        rotary.disableAcceleration(); //disable acceleration 
        //rotaryEncoder.setAcceleration(250); //or set the value - larger number = more accelearation; 0 or 1 means disabled acceleration
        return true;
        break;
      case 4: //GPS
        //choice of GPS off, on before tx, on all the time
        cur_menu = GPS_MENU;
        return true;
        break;
      case 5: //Run
    ...
    //a radio menu item is activated
    //activate the choice on the radio
    cur_menu = cmenu;
    ...
    switch (cur_menu) {
      case POWER_MENU:
        pPowerIndex = citem;
        preferences.putInt("powerIndex", pPowerIndex);
        //do for the radio as well
        break;
      case MOD_MENU:
}
        pModIndex = citem;
        preferences.putInt("modIndex", citem);
        //do for radio as well
        break;
      case ADD_MENU:
        pAddress = citem;
      ...
      case FREQ_MENU:
        pFreqIndex = citem;
        preferences.putInt("freqIndex", citem);
        //do for radio as well
        break;
      case GPS_MENU:
        pGpsIndex = citem;
        preferences.putInt("gpsIndex", citem);
        //do for radio as well
    }
